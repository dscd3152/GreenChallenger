...
public class MissionDetailActivity extends AppCompatActivity {
    ...
    private static final long VERIFICATION_TIMEOUT_MS = 60000L;
    ...
    private final ActivityResultLauncher<Intent> cameraLauncher =
            registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
                if (result.getResultCode() == RESULT_OK) {
                    ...
                    Toast.makeText(this, "사진 준비 완료! AI 검증을 요청해보세요.", Toast.LENGTH_SHORT).show();
                    isCompleted = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnCompleteMission.setOnClickListener(v -> {
            if (!isCompleted) {
                Toast.makeText(this, "사진 인증이 먼저 필요합니다.", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    private void uploadPhotoAndCompleteMission(String title) {
        ...
        String submissionId = String.valueOf(System.currentTimeMillis());

        btnCompleteMission.setEnabled(false);
        btnCompleteMission.setText("완료 여부 확인 중...");
        updateVerificationUi("reviewing", "완료 여부 확인 중", "오늘 이미 승인된 미션인지 확인하고 있습니다.");
        ...
        db.collection("users")
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    boolean pointsAlreadyAwarded = documentSnapshot.exists()
                            && Boolean.TRUE.equals(documentSnapshot.getBoolean("pointsAwarded"));
                    uploadPhotoToStorage(uid, missionId, title, today, docId, storagePath,
                            submissionId, pointsAlreadyAwarded);
                })
                .addOnFailureListener(e -> {
                    btnCompleteMission.setEnabled(true);
                    btnCompleteMission.setText("AI 검증 요청하기");
                    updateVerificationUi("failed", "확인 실패", e.getMessage());
                    Toast.makeText(this, "미션 상태 확인 실패: " + e.getMessage(), Toast.LENGTH_LONG).show();
                });
    }

    private void uploadPhotoToStorage(String uid, String missionId, String title, String today,
                                      String docId, String storagePath, String submissionId,
                                      boolean pointsAlreadyAwarded) {
    ...
    private void saveMissionToFirestore(String uid, String missionId, String title, String today,
                                        ...
                                        String submissionId, boolean pointsAlreadyAwarded) {
        ...
        db.runTransaction(transaction -> {
                ...
                .addOnSuccessListener(unused -> {
                    ...
                    listenVerificationResult(uid, docId, title, photoUrl, submissionId);
                })
                .addOnFailureListener(e -> {
                    btnCompleteMission.setEnabled(true);
                    btnCompleteMission.setText("AI 검증 요청하기");
    ...
    private void listenVerificationResult(String uid, String docId, String title, String photoUrl,
        ...
        verificationListener = db.collection("users")
                ...
                .addSnapshotListener((snapshot, error) -> {
                    ...
                    String latestSubmissionId = snapshot.getString("submissionId");

                    if (latestSubmissionId != null && !submissionId.equals(latestSubmissionId)) {
                        return;
                    }
    ...
    private void showExistingMissionStatus(String status, String reason) {
        if ("approved".equals(status)) {
            ...
            updateVerificationUi("idle", "다시 촬영 가능", reason != null ? reason : "기존 검증이 끝나지 않았습니다. 새 사진으로 다시 요청할 수 있습니다.");
            btnCompleteMission.setEnabled(false);
            btnCompleteMission.setText("AI 검증 요청하기");
            btnTakePhoto.setEnabled(true);
            isCompleted = false;
    ...
    private void startVerificationTimeout() {
        ...
        verificationTimeoutRunnable = () -> {
            updateVerificationUi(
                    "idle",
                    "검증 시간이 길어지고 있습니다",
                    "서버 응답이 늦어지고 있습니다. 잠시 후 결과가 반영될 수 있고, 필요하면 다시 촬영해 재요청할 수 있습니다."
            );
            btnTakePhoto.setEnabled(true);
            btnCompleteMission.setEnabled(false);
            btnCompleteMission.setText("AI 검증 요청하기");
        ...
        verificationHandler.postDelayed(verificationTimeoutRunnable, VERIFICATION_TIMEOUT_MS);
    }
    ...
    private String buildStorageErrorMessage(Exception exception) {
        if (exception instanceof StorageException) {
            ...
            if (errorCode == StorageException.ERROR_OBJECT_NOT_FOUND) {
                return "Storage 버킷 또는 업로드 경로를 찾을 수 없습니다.";
            }

            if (errorCode == StorageException.ERROR_NOT_AUTHENTICATED) {
                return "로그인 인증이 필요합니다. 다시 로그인해 주세요.";
            }
